    ...
    <script>
        ...
        $(document).ready(function() {
            $.getJSON("/sysconfig", function(data) {
                ...
                $("#intText").text("Sampling:" + data.interval + "ms");
            ...
            ws.onmessage = function(event) {
                ...
                if (event.data instanceof ArrayBuffer) {
                    ...
                    $.each(rows, function(index, value) {
                        ...
                        if (value[1] < 1200 && value[1] > -200) {
                            dat.push(value);
                            $('#temperature').text(value[1].toFixed(1));
                            $('#temperature').append("<span>°C</span>");
                        } else {
                            $('#temperature').text("Probe Error");
                        }
                    });
        ...
        function Raw2Arry(raw, struct) {
            ...
            while (i < dv.byteLength) {
                $.each(struct, function(key, val) {
                    switch (val.type) {
                        case "T":
                            //row.push(new Date(1000 * (dv.getUint32(i, "littleEndian"))));
                            //  i += 4

                            var utc = 1000 * (dv.getUint32(i, "littleEndian"))
                            i += 4
                            var ms = dv.getUint32(i, "littleEndian") % 1000;
                            i += 4
                            row.push(new Date(utc + ms));


                            break;
                        case "I8":
                            row.push(dv.getInt8(i) / val.DIV);
                            i += 1;
                            break;
                        case "U8":
                            row.push(dv.getUint8(i) / val.DIV);
                            i += 1;
                            break;
                        case "I16":
                            row.push(dv.getInt16(i, "littleEndian") / val.DIV);
                            i += 2;
                            break;
                        case "U16":
                            row.push(dv.getUint16(i, "littleEndian") / val.DIV);
                            i += 2;
                            break;
                        case "U32":
                            row.push(dv.getUint32(i, "littleEndian") / val.DIV);
                            i += 4;
                            break;
                        case "I32":
                            row.push(dv.getInt32(i, "littleEndian") / val.DIV);
                            i += 4;
                            break;
                        case "F32":
                            row.push(dv.getFloat32(i, "littleEndian") / val.DIV);
                            i += 4;
                            break;
                    }
                });
        ...
        function exportCSV(exportAll) {
            ...
            var st = 0;
            var csv = series.join(",") + "\r\n";
            csv = csv.replace("Date", "Relative time (sec)")
        ...
        function lockX() {
            if (lockX_time == 0)
                lockX_time = new Date().getTime() - 1000;
            else
                lockX_time = 0;
...
<body>
    <div data-role="page">
        <div data-role="header">
            ...
            <div id=recordDiv>
                <label id=recText>Recording... </label>
            </div>
            ...
            <div id=spaceDiv class="diskDiv">
                <label id=spaceText>Space Warning </label>
            </div>


            <div id=intDiv>

                <label id=intText>Sampling: 200ms</label>
            </div>

            <h1>Wireless Logger</h1>
        ...
        <div role="main" class="ui-content">
            <div id="display" class="ui-grid-a">
                <div id="display-l" class="ui-block-a">
                    <h1 id="temperature">Connecting....</h1>
                </div>
                <div id="display-r" class="ui-block-b">
            ...
            <div id="butDiv" class="ui-grid-b" data-position-fixed="true">
                ...
                <div class="ui-block-b"><a href="#" class="ui-shadow ui-btn ui-corner-all ui-mini">Mode</a></div>
